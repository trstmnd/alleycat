...
const bike = { x: 0, y: 0, heading: 0, slip: 0 };
for (let i = 0; i <= 60; i++) {
  bike.x = i * 10;
  bike.y = i;
  G.record(rec, i * G.STEP, bike);
}
ok(rec.samples.length >= 60 * G.STRIDE, 'trop peu d echantillons : ' + rec.samples.length);
ok(rec.samples.length % G.STRIDE === 0, 'echantillons incomplets');
...
ok(G.sampleAt(rec.samples, -1) === null, 'un temps negatif devrait rendre null');

// Le cap s interpole par le plus court chemin : sans ca le fantome fait un
// tour complet chaque fois qu il passe par pi.
const wrap = [0, 0, Math.PI - 0.1, 0, 0, 0, -Math.PI + 0.1, 0];
